...
func TestOpen_Sniff(t *testing.T) {
	...
	tests := []struct {
		...
		{"webp", Input{Stdin: bytes.NewReader([]byte("RIFF\x00\x00\x00\x00WEBPVP8L"))}, c2pa.RIFF, nil},
		{"wav", Input{Stdin: bytes.NewReader([]byte("RIFF\x00\x00\x00\x00WAVEfmt "))}, c2pa.RIFF, nil},
		{"tiff little-endian", Input{Stdin: bytes.NewReader([]byte("II*\x00\x08\x00\x00\x00"))}, c2pa.TIFF, nil},
		{"tiff big-endian", Input{Stdin: bytes.NewReader([]byte("MM\x00*\x00\x00\x00\x08"))}, c2pa.TIFF, nil},
		{"gif", Input{Stdin: bytes.NewReader([]byte("GIF89a............"))}, c2pa.GIF, nil},
		{"mp3", Input{Stdin: bytes.NewReader([]byte("ID3\x04\x00\x00\x00\x00\x00\x00"))}, c2pa.MP3, nil},
		{"svg", Input{Stdin: bytes.NewReader([]byte(`<svg xmlns="http://www.w3.org/2000/svg"></svg>`))}, c2pa.SVG, nil},
		{"unsupported", Input{Stdin: bytes.NewReader([]byte("PK\x03\x04 a zip, not an asset"))}, "", ErrUnsupportedFormat},
